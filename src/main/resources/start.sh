...
nohup java -Xmx256M -Xms64M -cp $CLASSPATH -Dpidfile=pid -Dconfigureme.defaultEnvironment=$CONFIGUREME_ENVIRONMENT $TARGET_JAR >log/stdout 2>log/stderr $TARGET_CLASS & echo $! > $TARGET_PID